...
type VolumesMetricsOfWorld interface {
	...
	MarkVolumeIsMeasuring(volumeName api.UniqueVolumeName)
	...
	UpdateVolumeMetricsData(volumeName api.UniqueVolumeName, volumeMetrics *volume.Metrics, outerVolumeSpecName string, cacheDuration time.Duration, dataTimestamp time.Time)

	// UpdateVolumeMetricsStatus update cache time and timestap
	// when fail to provide volume metrics.
	UpdateVolumeMetricsStatus(volumeName api.UniqueVolumeName, cacheDuration time.Duration, dataTimestamp time.Time)

	// HousekeepVolumeMetricsWorld removes the volume metrics from the
	// cache when the volume has not been in the actual world.
	HousekeepVolumeMetricsWorld(volumeNamesInActualWorld []api.UniqueVolumeName)
	...
	GetVolumeMetricsStatus(volumeName api.UniqueVolumeName) (measureRequired bool, cacheDuration *time.Duration)

	// GetVolumeMetrics return the volume metrics and volume outerVolumeSpecName.
	GetVolumeMetrics(volumeName api.UniqueVolumeName) (*volume.Metrics, string)
}
...
func NewVolumeMetricsOfWorld() VolumesMetricsOfWorld {
	return &volumeMetricsOfWorld{
		mountedVolumesMetricsCache: make(map[api.UniqueVolumeName]volumeMetricsData),
	}
}
...
type volumeMetricsOfWorld struct {
	mountedVolumesMetricsCache map[api.UniqueVolumeName]volumeMetricsData
	sync.RWMutex
}

func (vmw *volumeMetricsOfWorld) MarkVolumeIsMeasuring(volumeName api.UniqueVolumeName) {
	vmw.RLock()
	defer vmw.RUnlock()
...
func (vmw *volumeMetricsOfWorld) UpdateVolumeMetricsData(volumeName api.UniqueVolumeName,
...
func (vmw *volumeMetricsOfWorld) UpdateVolumeMetricsStatus(volumeName api.UniqueVolumeName,
	cacheDuration time.Duration,
	dataTimestamp time.Time) {
	vmw.RLock()
	defer vmw.RUnlock()
...
func (vmw *volumeMetricsOfWorld) HousekeepVolumeMetricsWorld(volumeNamesInActualWorld []api.UniqueVolumeName) {
	vmw.RLock()
	defer vmw.RUnlock()
...
func (vmw *volumeMetricsOfWorld) GetVolumeMetricsStatus(volumeName api.UniqueVolumeName) (bool, *time.Duration) {
	vmw.RLock()
	defer vmw.RUnlock()
...
func (vmw *volumeMetricsOfWorld) GetVolumeMetrics(volumeName api.UniqueVolumeName) (*volume.Metrics, string) {
	vmw.RLock()
	defer vmw.RUnlock()
