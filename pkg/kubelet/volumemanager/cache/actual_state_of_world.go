...
type ActualStateOfWorld interface {
	...
	// state of the world.
	GetMountedVolumeNames() []api.UniqueVolumeName
...
func (asw *actualStateOfWorld) GetMountedVolumeNamesForPod(podName volumetypes.UniquePodName) []api.UniqueVolumeName {
	asw.RLock()
	defer asw.RUnlock()
	mountedVolumeNames := make([]api.UniqueVolumeName, 0 /* len */, len(asw.attachedVolumes) /* cap */)
	for _, volumeObj := range asw.attachedVolumes {
...
func (asw *actualStateOfWorld) GetMountedVolumeNames() []api.UniqueVolumeName {
	asw.RLock()
	defer asw.RUnlock()
	mountedVolumeNames := make([]api.UniqueVolumeName, 0 /* len */, len(asw.attachedVolumes) /* cap */)
	for _, volumeObj := range asw.attachedVolumes {
